...
function pageInit() {
	...
	var toolbar1 = {
	        parent: "ToolBar1",
	        items: [{type: "text",id: "adds_bt1",text: "统计指标结果"}]};
	var toolbar2 = {
	        parent: "ToolBar2",
	        items: [{type: "text",id: "adds_bt2",text: "30天入库趋势图"}]};
	var toolbar3 = {
	        parent: "ToolBar3",
	...
	var toolbar5 = {
	        parent: "ToolBar5",
	        items: [{type: "text",id: "adds_bt5",text: "按日统计慢查询排行（耗时排名）"}]};
	var toolbar6 = {
	        parent: "ToolBar6",
	        items: [{type: "text",id: "adds_bt6",text: "24小时统计指标结果"}]};
	var toolbar7 = {
	        parent: "ToolBar7",
	        items: [{type: "text",id: "adds_bt7",text: "统计指标>调用次数"}]};
    new dhtmlXToolbarObject(toolbar1);
    ...
    var myCalendar = new dhtmlXCalendarObject("dateNo");
...
function selectPart(rid){
	...
	LogAnalysisAction.queryPartRs(queryParam,{
			async:false,
	        callback:function(data){
...
function getDataXml(data,unit,rsName){
	var zoneTimeDataXml = "<chart  numVDivLines='22' yAxisMinValue='0' yAxisMaxValue='10'  " +
			"bgColor='FFFFFF' defaultNumberScale='' numberScaleUnit='万,亿,兆' " +
			"numberScaleValue='10000,10000,10000'  " +
	...
	zoneTimeDataSuccess += getHour(data);
 ...
 * @returns {String}
 */
function getHour(data,flag){
	var rs ="";
	for(var k=0;k<arrHour.length;k++){
		var booleanCheck = 0;
		for(var i=0;i<data.length;i++){
			if(arrHour[k]==data[i].T_HOUR){
				booleanCheck =1;
				rs += " <set value='"+data[i].T_COUNT+"' /> ";
				continue;
			}
		}
		if(booleanCheck==0){
...
function query(){
	...
	LogAnalysisAction.showAllData(termVals,function(data){
...
function getXmlData(data){
	...
	strXML = "<chart canvasPadding='10' caption='"+(data.typeName==null?"":data.typeName)+"' yAxisName='入库条数(条)' bgColor='FFFFFF'" +
			" numVDivLines='28' divLineAlpha='30' labelDisplay='ROTATE'  labelPadding ='10' yAxisValuesPadding ='10' " +
			"showValues='0' defaultNumberScale='' yAxisMinValue='0' yAxisMaxValue='10' numberScaleUnit='万,亿,兆' numberScaleValue='10000,10000,10000' rotateValues='1' valuePosition='auto'" +
			">";
	strXML += "<categories>";
...
function refreshLineChar(data){
	var chartObj = getChartFromId("ChLineId");
	var dataXml = getXmlData(data); 
    chartObj.setDataXML(dataXml);
}

function buildLineTopChar(){
	var termVals = {};
	termVals["jobType"] = document.getElementById("job_type").value==""?firstTypeId:document.getElementById("job_type").value;
    termVals["dateNo"] = document.getElementById("dateNo").value;
    var sObj =document.getElementById("job_type");
    termVals["typeName"] =sObj.options[sObj.selectedIndex].text==""?firstTypeName:sObj.options[sObj.selectedIndex].text;
	LogAnalysisAction.getLogAnalysisLineTop(termVals,function(data){
		lineTopChar(data);
	});
}
...
function refreshTopChar(data){
	var topTempXml = getTopCharXML(data);
	var chartTop =  getChartFromId("ChTopId");
	chartTop.setDataXml(data);
}
...
function getTopCharXML(data){
	...
	strXML = "<chart canvasPadding='10' caption='"+(data.typeName==null?"":data.typeName)+"' labelDisplay='ROTATE' yAxisName=''" +
			...
			" rotateValues='1' valuePosition='auto' " +
			" >";
	strXML += "<categories>";
	var arrDatanos = data.datanos;
	for(var i=0;i<arrDatanos.length;i++){
...
function queryData(dt,params){
    ...
    termVals["dateNo"] = document.getElementById("dateNo").value;
	termVals["jobType"] = document.getElementById("job_type").value==""?firstTypeId:document.getElementById("job_type").value;
	LogAnalysisAction.getLogAnalysisDetail(termVals,{posStart:params.page.rowStart,count:params.page.pageSize},function(data){
        var total = 0;
        if(data && data[0])
...
function queryTopData(dt,params){
    ...
    termVals["dateNo"] = document.getElementById("dateNo").value;
	termVals["jobType"] = document.getElementById("job_type").value==""?firstTypeId:document.getElementById("job_type").value;
    LogAnalysisAction.getLogAnalysisTopDetail(termVals,{posStart:params.page.rowStart,count:params.page.pageSize},function(data){
    	var total = 0;
        if(data && data[0])
            total = data[0]["TOTAL_COUNT_"];
        	//查询出数据后，必须显示调用绑定数据的方法
...
function queryInputData(dt,params){
    ...
    termVals["dateNo"] = document.getElementById("dateNo").value;
	termVals["jobType"] = document.getElementById("job_type").value==""?firstTypeId:document.getElementById("job_type").value;
    LogAnalysisAction.getInputDetail(termVals,function(data){
        if(data){
...
function download(){
	...
	var image = getRsList("image");//得到入库趋势列表
	...
	if(downtype==""){
    	...
    	var url  =  'downloadDayWord.jsp?type=1&dateTime='+date+'&rs='+rs+'&image='+image+'&rule='+rule+'&srule='+srule+'&lstTypeId='+lstTypeId;
		location.href = url;
