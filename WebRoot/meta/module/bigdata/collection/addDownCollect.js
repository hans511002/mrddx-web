...
function pageInit() {
	...
	if(flag!='add'){
		CollectionAction.queryJobById(col_id, function(rs){
	        dhx.closeProgress();
			...
			document.getElementById("COL_TASK_PRIORITY").value=rs.COL_TASK_PRIORITY==null?"":rs.COL_TASK_PRIORITY;
			...
			document.getElementById("COL_TASK_NUMBER").value=rs.COL_TASK_NUMBER==null?"":rs.COL_TASK_NUMBER;
			document.getElementById("COL_SYS_INPUTPATH").value=rs.COL_SYS_INPUTPATH==null?"":rs.COL_SYS_INPUTPATH;
			document.getElementById("COL_DESCRIBE").value=rs.COL_DESCRIBE==null?"":rs.COL_DESCRIBE;
			document.getElementById("COL_TYPE").value=rs.COL_TYPE==null?"":rs.COL_TYPE;
			
			document.getElementById("OUTPUT_DATASOURCE_ID").value=rs.OUTPUT_DATASOURCE_ID==null?"":rs.OUTPUT_DATASOURCE_ID;
			document.getElementById("OUTPUT_DATASOURCE_NAME").value=rs.OUTPUT_DATASOURCE_NAME==null?"":rs.OUTPUT_DATASOURCE_NAME;
			document.getElementById("OUTPUT_PATH").value=rs.OUTPUT_PATH==null?"":rs.OUTPUT_PATH;
    ...
    //选择数据源
    var source = document.getElementById("OUTPUT_DATASOURCE_NAME");
    attachObjEvent(source,"onclick",function(){
    	openDataSourceTableWin(this,"OUTPUT_DATASOURCE_ID","OUTPUT_DATASOURCE_NAME"); 
    });
    var source2 = document.getElementById("INPUT_DATASOURCE_NAME");
    attachObjEvent(source2,"onclick",function(){
    	openDataSourceTableWin(this,"INPUT_DATASOURCE_ID","INPUT_DATASOURCE_NAME"); 
    });
    var source3 = document.getElementById("INPUT_FILELST_DATASOURCE_NAME");
    attachObjEvent(source3,"onclick",function(){
    	openDataSourceTableWin(this,"INPUT_FILELST_DATASOURCE_ID","INPUT_FILELST_DATASOURCE_NAME"); 
    }); 
    ...
    /**	if(document.getElementById("COL_NAME").value.length>50){
    		 dhx.alert("采集名称过长");
    		 return;
    	}**/
		...
		if(document.getElementById("INPUT_FILELST_TYPE").value==1&&document.getElementById("INPUT_FILELST_DATASOURCE_ID").value==""){
    		 dhx.alert("请选择文件列表来源数据源");
    		 return;
    	}
		if(document.getElementById("INPUT_FILELST_TYPE").value==1&&document.getElementById("INPUT_QUERY_SQL").value==""){
    		 dhx.alert("请输入文件列表查询SQL语句");
    		 return;
    	}
		if(document.getElementById("INPUT_FILELST_TYPE").value==0&&document.getElementById("INPUT_PATH").value==""){
    		 dhx.alert("请输入输入目录");
    		 return;
    	}
		...
		if((document.getElementById("INPUT_DOTYPE").value==2||document.getElementById("INPUT_DOTYPE").value==3)&&document.getElementById("INPUT_MOVE_PATH").value==""){
    		 dhx.alert("请输入输入文件移动目录");
    		 return;
    	}
		if((document.getElementById("INPUT_DOTYPE").value==4||document.getElementById("INPUT_DOTYPE").value==3)&&document.getElementById("INPUT_RENAME_RULE").value==""){
    		 dhx.alert("请输入输入文件重命名规则");
    		 return;
    	}
		if(document.getElementById("INPUT_RENAME_RULE").value!=""&&document.getElementById("INPUT_RENAME").value==""){
		...
		if(document.getElementById("OUTPUT_RENAME_RULE").value!=""&&document.getElementById("OUTPUT_RENAME").value==""){
		...
		allData.COL_TASK_PRIORITY = document.getElementById("COL_TASK_PRIORITY").value;
		allData.COL_SYS_INPUTPATH = document.getElementById("COL_SYS_INPUTPATH").value;
		allData.COL_TYPE = document.getElementById("COL_TYPE").value;
		
		allData.INPUT_DATASOURCE_ID = document.getElementById("INPUT_DATASOURCE_ID").value;
		allData.INPUT_FILELST_TYPE = document.getElementById("INPUT_FILELST_TYPE").value;
		allData.INPUT_FILELST_DATASOURCE_ID = document.getElementById("INPUT_FILELST_DATASOURCE_ID").value;
		...
		allData.INPUT_RENAME_RULE = document.getElementById("INPUT_RENAME_RULE").value;
		allData.INPUT_RENAME = document.getElementById("INPUT_RENAME").value;
		
		allData.OUTPUT_DATASOURCE_ID = document.getElementById("OUTPUT_DATASOURCE_ID").value;
		allData.OUTPUT_PATH = document.getElementById("OUTPUT_PATH").value;
		allData.OUTPUT_RENAME_RULE = document.getElementById("OUTPUT_RENAME_RULE").value;
...
function cleanForm(){
 		...
 		document.getElementById("INPUT_DATASOURCE_ID").value = "";
		document.getElementById("INPUT_DATASOURCE_NAME").value = "";
		document.getElementById("INPUT_FILELST_TYPE").value = "";
		document.getElementById("INPUT_FILELST_DATASOURCE_ID").value = "";
		document.getElementById("INPUT_FILELST_DATASOURCE_NAME").value = "";
		document.getElementById("INPUT_QUERY_SQL").value = "";
		document.getElementById("INPUT_PATH").value = "";
		document.getElementById("INPUT_FILE_RULE").value = "";
		document.getElementById("INPUT_DOTYPE").value = "";
		...
		tid = 0;
		//document.getElementById("add_CleanBtn").style.display = "none";
...
function addParamGridInit(){
	...
	dataListTable.setColumns({
        INPUT_DATASOURCE_ID:"输入数据源ID",
        INPUT_DATASOURCE_NAME:"输入数据源名称",
        INPUT_FILELST_TYPE:"文件列表类型",
        OUTPUT_RENAME_RULE:"输出文件重命名规则",
        INPUT_FILE_RULE:"输入文件规则",
        INPUT_DOTYPE:"输入文件处理类型",
        OPP:"操作"
    },"INPUT_DATASOURCE_ID,INPUT_DATASOURCE_NAME,INPUT_FILELST_TYPE,OUTPUT_RENAME_RULE,INPUT_FILE_RULE,INPUT_DOTYPE,ID,COL_ID,INPUT_FILELST_DATASOURCE_ID,INPUT_QUERY_SQL,INPUT_PATH,INPUT_MOVE_PATH,INPUT_RENAME_RULE,NOTE,OUTPUT_DATASOURCE_ID,OUTPUT_PATH,OUTPUT_MOVE_PATH,INPUT_FILELST_DATASOURCE_NAME,OUTPUT_DATASOURCE_NAME,INPUT_SOURCE_NAME,INPUT_FILELST_SOURCE_NAME,OUTPUT_SOURCE_NAME");
	dataListTable.setPaging(true,20);//分页
	dataListTable.setRowIdForField("ID");
	dataListTable.render();
	if(flag=="info"){
		dataListTable.grid.setInitWidthsP("15,15,15,15,15,25,0");
	}else{
		dataListTable.grid.setInitWidthsP("15,15,15,15,15,20,5");
	}
    dataListTable.setGridColumnCfg(0,{align:"left"});
    dataListTable.setGridColumnCfg(1,{align:"left"});
    dataListTable.setGridColumnCfg(2,{align:"left"});
    dataListTable.setGridColumnCfg(3,{align:"left"});
    dataListTable.setGridColumnCfg(4,{align:"left"});
    dataListTable.setGridColumnCfg(5,{align:"left"});
    dataListTable.setGridColumnCfg(6,{align:"left"});
    ...
    dataListTable.setFormatCellCall(function(rid,cid,data,colId){
    	...
    	if(colId=="INPUT_DATASOURCE_NAME"){
    		return "<a href='javascript:void(0)' onclick='onrowclick("+rid+");return false;'>"+data[cid]+"</a>";
    	}
    	if(colId=="INPUT_DATASOURCE_ID"){
    		return "<a href='javascript:void(0)' onclick='openViewDataSource("+dataListTable.getUserData(rid,"INPUT_DATASOURCE_ID")+",\""+dataListTable.getUserData(rid,"INPUT_DATASOURCE_NAME")+"\",\""+dataListTable.getUserData(rid,"INPUT_SOURCE_NAME")+"\");return false;'>"+data[cid]+"</a>";
    	}
        return data[cid];
    });
