...
function pageInit() {
    ...
    var reduceTaskId = termReq.createTermControl("reduceTaskId","REDUCE_TASK_ID");
    reduceTaskId.setInputEnterCall(function(){
    ...
    attachObjEvent(rollbackBtn,"onclick",function(){
    	window.parent.closeTab("查看Reduce详细日志");
...
function dataTableInit(){
    ...
    dataTable.setColumns({
        LOG_ID:"LogID",
        REDUCE_TASK_ID:"Reduce任务ID",
        LOG_TYPE_NAME:"日志类型",    	
        LOG_DATE:"生成日志时间",
        LOG_MSG:"日志详情"
    },"LOG_ID,REDUCE_TASK_ID,LOG_TYPE_NAME,LOG_DATE,LOG_MSG,LOG_TYPE");
    ...
    dataTable.grid.setInitWidthsP("10,30,10,20,30");
...
function queryData(dt,params){
    ...
    termVals.REDUCE_TASK_ID2 = reduceTaskId;
    dhx.showProgress("请求数据中");
    JobLogAction.queryReduceMsgLog(termVals,{posStart:params.page.rowStart,count:params.page.pageSize},function(data){
        dhx.closeProgress();
        var total = 0;
        if(data && data[0])
