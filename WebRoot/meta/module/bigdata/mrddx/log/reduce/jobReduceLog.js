...
function dataTableInit(){
    ...
    dataTable.setColumns({   	
        ...
        LOG_MSG:"日志信息",
        opt:"操作"
    },"LOG_ID,REDUCE_TASK_ID,REDUCE_INPUT_COUNT,REDUCE_OUTPUT_COUNT,START_DATE,END_DATE,RUN_FLAG_NAME,LOG_MSG,RUN_FLAG");
//    dataTable.setRowIdForField("LOG_ID");
    ...
    dataTable.setFormatCellCall(function(rid,cid,data,colId){
        ...
        if(colId=="OPT"){
            var reduceTaskId = dataTable.getUserData(rid,"REDUCE_TASK_ID");
            var str = "";
            //操作
            str += "&nbsp;<a href='javascript:void(0)' onclick='OpenViewReduceWin(\""+reduceTaskId+"\");return false;'>查看Reduce详细日志</a>";
            return str;
        }
        return data[cid];
    });
...
function queryData(dt,params){
    ...
    dhx.showProgress("请求数据中");
    JobLogAction.queryReduceLog(termVals,{posStart:params.page.rowStart,count:params.page.pageSize},function(data){
        dhx.closeProgress();
        var total = 0;
        if(data && data[0])
...
function OpenViewReduceWin(b){
	var url = "/meta/module/bigdata/mrddx/log/reduce/jobReduceMsgLog.jsp?reduceTaskId="+b;
	try{
		parent.openMenu("查看Reduce详细日志",url,"top","reduceTaskId="+b);	
	}catch(e){
		window.open(urlEncode(getBasePath()+url),'reduceTaskId'+b);
	}
