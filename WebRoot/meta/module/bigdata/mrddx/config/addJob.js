...
function jobAddWin (){
    ...
    attachObjEvent(resetBtn,"onclick",function(){    
    	document.getElementById("jobName").value="";
    	document.getElementById("jobPriority").value="";
		document.getElementById("mapTasks").value="";
		document.getElementById("reduceTasks").value="";
		document.getElementById("JOB_RUN_DATASOURCE").value="";
		document.getElementById("JOB_RUN_DATASOURCE_NAME").value="";
		document.getElementById("inputDir").value="";
		document.getElementById("jobDescribe").value="";

		inputPageInit(false);
    	outputPageInit(false);
    	systemPageInit(false);
    ...
    attachObjEvent(saveBtn,"onclick",function(){
        ...
        if(!(dhtmlxValidation.validate("_jobConfigParamForm")))return;
			...
			JobAction.insertJob(allData, function(rs){
		        dhx.closeProgress();
		        if(rs){
		            dhx.alert("保存成功！",function(){
		            	window.parent.closeTab("新增任务");
		            });
...
function dataSourceInputInit(){
    ...
    dataTableInput.setPaging(true,9);//分页
	...
	dataTableInput.setFormatCellCall(function(rid, cid, data, colId) {
		if (colId == "RA") {
			if(allData.dataInput!=null){
				if(allData.dataInput["DATA_SOURCE_ID"]==data[1]){
					return 1;
				}
			}
		}
...
// ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff第二步dfffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff 
//输出数据源：初始化界面
function secondInit() {
...
function dataSourceOutputInit(){
    ...
    dataTableOutput.setPaging(true,9);//分页
	...
	dataTableOutput.setFormatCellCall(function(rid, cid, data, colId) {
		if (colId == "RA") {
		if(allData.dataOutput!=null){
				if(allData.dataOutput["DATA_SOURCE_ID"]==data[1]){
					return 1;
				}
			}
		}
...
// ggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggg第三步gggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggg 
var INPUT_SOURCE_TYPE_ID = '';
function inputPageInit(flag) {
    ...
    if(flag&&INPUT_SOURCE_TYPE_ID==dataInput.SOURCE_TYPE_ID){
    	return;
    }else{
    	INPUT_SOURCE_TYPE_ID=dataInput.SOURCE_TYPE_ID;
    }
    paramInputTable.setReFreshCall(
	    JobAction.querySourceParam(termVals,function(data){
	        dhx.closeProgress();
...
function paramInputInit(){
    ...
    paramInputTable.setFormatCellCall(function(rid,cid,data,colId){
    	...
    	if(colId=="DEFAULT_VALUE"){
			...
			var str = '<input value="'+paramInputValue+'" style="width: 300px" id="paramInputValue'+rid+'" title="'+rid+'" ></input>';
			return str;
    	}
    	if(colId=="PARAM_DESC"){
...
function outputPageInit(flag) {
    ...
    if(flag&&OUTPUT_SOURCE_TYPE_ID==dataOutput.SOURCE_TYPE_ID){
    	return;
    }else{
...
function paramOutputInit(){
    ...
    paramOutputTable.setColumns({
        ...
        PARAM_DESC:"参数描述",
        OPP:''
    },"PARAM_NAME,'',PARAM_DESC,DATA_SOURCE_ID");
    ...
    paramOutputTable.setFormatCellCall(function(rid,cid,data,colId){
    	...
    	if(colId=="DEFAULT_VALUE"){
			...
			var str = '<input value="'+paramOutputValue+'" style="width: 300px" id="paramOutputValue'+rid+'" title="'+rid+'" ></input>';
			return str;
    	}
    	if(colId=="PARAM_DESC"){
...
function systemPageInit(flag) {
    ...
    paramSystemTable.setReFreshCall(
	    JobAction.querySystemParam(termVals,function(data){
	        dhx.closeProgress();
	        paramSystemTable.bindData(data);
	        //alert(data[0].IS_MUST);
...
function paramSystemInit(){
    ...
    paramSystemTable.setFormatCellCall(function(rid,cid,data,colId){
    	...
    	if(colId=="DEFAULT_VALUE"){
			...
			var str = '<input value="'+paramSysValue+'" style="width: 300px" id="paramSysValue'+rid+'" title="'+rid+'" ></input>';
			return str;
    	}
    	if(colId=="PARAM_DESC"){
