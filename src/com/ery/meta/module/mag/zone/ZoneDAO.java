...
public class ZoneDAO extends BaseDAO {
	...
	/**
	 * 加载从起始节点到结束节点之间有路径关系节点的所有数据，而不是加载从起始节点到结束节点之间所有的节点数据。 比如可以查询起始地域为0，结束地域为7之间树集关系的所有的数据，而地域7必然为地域0下的地域。
	 * 
	 * 
	 * @param beginId
	 *            起始地域ID。
	 * @param endId
	 *            结束地域ID，如=0，只查找从指定起始节点下的两层树形数据。
	 * @return
	 */
	...
	public Map<String, Object> queryZoneInfo(int zoneId) {
		String sql = " SELECT A.ZONE_ID,A.ZONE_PAR_ID,A.ZONE_NAME,A.ZONE_DESC,A.DIM_TYPE_ID, A.DIM_TYPE_ID,A.DIM_LEVEL,"
				...
				+ " FROM META_DIM_ZONE A LEFT JOIN META_DIM_ZONE B on  A.ZONE_PAR_ID = B.ZONE_ID "
				+ " WHERE A.ZONE_ID = ? AND A.DIM_TYPE_ID=4";
